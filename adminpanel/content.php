...
if(!empty($_FILES['limage']['name'])) 
{
	...
	$image = time().$image;
	$originalpath = "../webupload/original/content/".$image;
	$thumbnailpath = "../webupload/thumb/content/".$image;
	move_uploaded_file($source,$originalpath);

		if($width<414||$height<324)
	{
		$parts=explode(".",$image);
		$profile_image=$parts[0].".png";
		include_once('imgresize.php');
		resize($originalpath, $thumbnailpath, 414, 324);
	}
	else
	{ 
		include('resize.php');
...
$field=array();	
$field['ldate']=get_entity($_REQUEST['ldate']);	
$field['lcontent']=get_entity($_REQUEST['lcontent']);	
$field['limage']=$image;
$field['lnumber']=get_entity($_REQUEST['lnumber']);	
$sid = $_REQUEST["mid"];
...
<div class="content-wrapper">
		...
		<? if(($_REQUEST['action']=="add") || ($_REQUEST['action']=="edit"))
			...
			<form name="form" id="pageform" class="form-horizontal pageform" enctype="multipart/form-data" method="post" action="">
             ...
             <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Date<font color="red">*</font> :</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="ldate" name="ldate" placeholder="Date" value="<?=get_symbol($res_edit[0]["ldate"])?>" >
                  </div>
              </div>

				
               <div class="form-group">
                 <label for="inputEmail3" class="col-sm-2 control-label">Description<font color="red">*</font>:</label>
                  <div class="col-sm-10">
                   <textarea id="lcontent" name="lcontent" placeholder="Description"  rows="10" cols="83" style=" border: 1px solid #000;"><?=get_symbol($res_edit[0]["lcontent"])?></textarea>
					<label for="lcontent" id="mcontenterrr" class="error" style="display:none;">Please enter description</label>
                  </div>
                </div> 	
                ...
                <? if($sid==1) { ?>
				  ...
				  <input type="hidden" value="<?=$res_edit[0]['limage']?>" id="theValue" name="theValue">				  
					...
					<br/><font color="#090"><b>Allowed JPG/PNG Image with size 414x324 pixels.</b></font>
                 <? if($res_edit[0]['limage']!=''){ ?>    
                             <br/> <img src="image.php?width=100&amp;height=50&amp;cropratio=1.0:1.0&amp;image=<?=$sitepath?>webupload/thumb/content/<?=$res_edit[0]['limage']; ?>" border="0" class="imgborder"  style="vertical-align:top" alt="Content"  /> 
				 <? } ?>
                  </div>
                </div> <? } ?>
                <? if($sid==2) { ?>
                <!-- <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Phone Number<font color="red">*</font> :</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="enumber" name="enumber" placeholder="Phone Number" value="<?=get_symbol($res_edit[0]["lnumber"])?>" >
                  </div>
              </div>
              ...
              <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="e_mail" name="e_mail" placeholder="Email" value="<?=get_symbol($res_edit[0]["e_mail"])?>" ><font color="#090"><b>[Ex: info@yourmail.com]</b></font>
                  </div>
              </div>	 -->
          <? } ?>
			...
			<form name="form" id="pageform" class="form-horizontal pageform" enctype="multipart/form-data" method="post" action="">
				 ...
				 <div class="form-group">
                  ...
                  <div class="col-xs-10 viewtext"><?=get_symbol($res_view[0]["ldate"])?></div>  
                 </div>
				...
				<div class="form-group">
                  ...
                  <div class="col-xs-10 viewtext"> <img src="image.php?width=400&amp;height=200&amp;cropratio=1:1&amp;image=<?=$sitepath?>webupload/thumb/content/<?=$res_view[0]['limage']; ?>" border="0" class="imgborder"  style="vertical-align:top" alt="aboutus"  /></div>  
				</div>	<? } ?>

				<? if($sid==2) { ?>
				<!-- <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Phone Number :</label>
                  <div class="col-xs-10 viewtext"><?=get_symbol($res_view[0]["lnumber"])?></div>  
                </div>

                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Email :</label>
                  <div class="col-xs-10 viewtext"><a href="mailto:<?=get_symbol($res_view[0]["e_mail"])?>"><?=get_symbol($res_view[0]["e_mail"])?></a></div>  
				</div>
				<? } ?> -->
	...
	<!--	 <a href="<?=$pagename?>?action=add"><button type="submit" class="btn btn-success pull-right"><i class="fa fa-plus-square"></i>&nbsp;&nbsp;Add Manage</button></a> -->
            ...
            <div class="box-body">			
              <table id="datatables" class="table table-bordered table-striped">
                ...
                <tr>
				 ...
				 <td><?=get_symbol($res['ldate'])?></td>	   
				  <td><? if(strlen($res['lcontent']) > 40){ echo strip_tags(substr(get_symbol($res['lcontent']),0,40)."...");}else{echo strip_tags(get_symbol($res['lcontent']));}?></td>
...
$(function () {
	...
	$("#pageform").validate({
          ...
          debug: false,
		rules: {	
			lcontent: { required: function() 
                        {
                         CKEDITOR.instances.lcontent.updateElement();
                        },minlength:10},  
			ldate: {required: true},
			
		},
		messages: {	
			ldate:{required: "Please enter date"},
			lcontent:{required:"Please enter content"},
		},
			
		submitHandler: function() {	

var description = CKEDITOR.instances['lcontent'].getData().replace(/<[^>]*>/gi, '');
			var file = $('#theValue').val();	
